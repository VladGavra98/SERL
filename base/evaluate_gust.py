#"""  Validation for the External Disturbance script """
import argparse
...
from evaluation_utils import Stats, load_pop, load_rl_agent, find_logs_path
...
parser.add_argument('-env', help='Environment Choices: (LunarLanderContinuous-v2) (PHLab)',
                    type=str, default='PHlab_attitude_gust')
...
def evaluate(env, actor, **kwargs: dict):
    ...
    plt.plot(57.3 * np.asarray(alpha_lst))
...
def validate_agent(gen_agent, user_refs_lst: list, num_trails: int = 1, **kwargs) -> tuple:
    """ Run evaluation over the user specified number of trails.
        ...
        NOTE due to the .SO binary not reseting the global time variable,
         only one evalaution can be performed!
    """
    ...
    env.seed(cla.seed)
    ref_tup = user_refs_lst[0]
    user_refs = {
        'theta_ref': ref_tup[0],
        'phi_ref': ref_tup[1],
    }
    data, _nmae, _smoothness = evaluate(env,
...
# def validate_actor(gen_agent, user_refs_lst: list, num_trails: int = 1, **kwargs) -> tuple:
#     """ Run evaluation over the user specified number of trails.
#         The time traces from data come from he LAST episode played.
#     """
#     agent_nmae_lst, agent_sm_lst = [], []
...
#     # Identify fault:
#     fault_name = 'gust'
#     assert cla.env.split('_')[-1] == fault_name

#     '''         Specify name of environment         '''
#     path_logs = 'logs/wandb/run-20220924_144643-1xzaqiba_LONG'

#     if 'zesty' or 'rl' in path_logs.lower(): eval_rl = True
...
#     # Pitch set-points:
#     amp1 = [0.2, 15, 3, -4, -8, 2]
#     # amp1 = [0,45,40,15,0,0]   # -- Perter's stall ref
#     base_ref_theta = signals.SmoothedStepSequence(
#         time_array, amp1, smooth_width=t_max//10)

#     # Roll set-points:
#     amp2 = [2, -2, 2, 10, 2, -6]
#     base_ref_phi = signals.SmoothedStepSequence(time_array, amp2, smooth_width=t_max//10)
#     # base_ref_phi = signals.Const(0, t_max,0.0) #NOTE overwriting the roll ref
...
#         '''              Load Population               '''
#         # Laod agent info
#         logs_dir     = extract_model_dir(path_logs)
#         model_config = load_config(logs_dir)
...
#         '''              Evaluation of one Actor              '''
#         data, stats = validate_actor(
#             pop[cla.index], user_eval_refs, cla.num_trails, stdout = True)

#     else:
#         '''              RL   Evaluation               '''

#         # Load RL agent from logs
#         logs_dir = extract_model_dir(path_logs)
#         model_config = load_config(logs_dir)
...
#         rl_agent = load_rl_agent(logs_dir, args=rl_parameters)
#         data, stats = validate_actor(
#             rl_agent, user_eval_refs, cla.num_trails, stdout=True)


#     '''              Load Nominal Trajectory               '''
#     try:
#         nominal_data = np.genfromtxt(figpath / 'nominal/nominal_trajectory.csv')
#     except OSError:
#         nominal_data = np.zeros_like(data)


#     _fig, _ = plot_diff(data, nominal_data, name=f'nMAE: {stats.nmae:0.1f}%      Smoothness: {stats.sm:0.0f}', fault=fault_name)
...
#         if not os.path.exists(faultpath):
#             os.mkdir(faultpath)
#         figname = faultpath / Path(f'actor{cla.index}'+'_' + fault_name + '.png')
#         _fig.savefig(fname=figname, dpi=400, format='png')
#         plt.close()
#     else:
#         plt.show()
